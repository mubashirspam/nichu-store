                ...
                <AnimatePresence>
                  {showUserMenu && (
                    <motion.div initial={{ opacity: 0, y: 8, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8, scale: 0.95 }}
                      ...
                      <button onClick={async () => { await signOut(); setShowUserMenu(false); }} className={`flex items-center gap-3 px-4 py-2.5 text-sm text-red-500 w-full ${d ? "hover:bg-white/5" : "hover:bg-red-50"}`}>
